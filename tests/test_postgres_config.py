...
def test_default_values():
    ...
    assert config.connection_string == "postgresql+psycopg2://postgres:postgres@localhost:5432/postgres"


def test_custom_values():
    config = PostgresConfig(
        user="custom_user", password="custom_password", host="custom_host", port=1234, database="custom_database"
    )
    ...
    assert (
        config.connection_string == "postgresql+psycopg2://custom_user:custom_password@custom_host:1234/custom_database"
    )
...
def test_from_env(monkeypatch):
    ...
    assert config.connection_string == "postgresql+psycopg2://env_user:env_password@env_host:5678/env_database"
