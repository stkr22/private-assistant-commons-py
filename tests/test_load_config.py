...
def test_load_single_yaml_file(temp_yaml_files):
    ...
    config = skill_config.load_config(single_file)
...
def test_load_multiple_yaml_files(temp_yaml_files):
    config = skill_config.load_config(temp_yaml_files)
    assert config.mqtt_server_host == "test_host"
    assert config.mqtt_server_port == 1884
...
def test_load_nonexistent_file():
    with pytest.raises(FileNotFoundError):
        skill_config.load_config("nonexistent.yaml")
...
def test_validation_error(temp_yaml_files):
    ...
    with pytest.raises(ValidationError):
        skill_config.load_config(invalid_yaml_file)
