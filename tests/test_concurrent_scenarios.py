...
class TestConcurrentSkill(BaseSkill):
    ...
    async def calculate_certainty(self, _intent_request: intent.IntentRequest) -> float:
...
class TestConcurrentProcessing:
    ...
    async def test_concurrent_message_processing(self):
        ...
        for i in range(message_count):
            client_request = intent.ClientRequest(
                id=uuid.uuid4(), text=f"test message {i}", room="test_room", output_topic="test/output"
            )
            classified_intent = intent.ClassifiedIntent(
                intent_type=intent.IntentType.DEVICE_ON,
                confidence=0.95,
                entities={},
                raw_text=f"test message {i}",
            )
            intent_request = intent.IntentRequest(
                classified_intent=classified_intent,
                client_request=client_request,
            )
            test_messages.append(intent_request.model_dump_json())

        # Process all messages concurrently
        tasks = [skill.handle_client_request_message(msg) for msg in test_messages]
        ...
        assert len(skill.intent_requests) == message_count
    ...
    async def test_memory_leak_prevention(self):
        ...
        skill.intent_requests.max_size = CACHE_SIZE_LIMIT
        ...
        for i in range(message_count):
            client_request = intent.ClientRequest(
                id=uuid.uuid4(), text=f"cache test {i}", room="test_room", output_topic="test/output"
            )
            classified_intent = intent.ClassifiedIntent(
                intent_type=intent.IntentType.DEVICE_ON,
                confidence=0.95,
                entities={},
                raw_text=f"cache test {i}",
            )
            intent_request = intent.IntentRequest(
                classified_intent=classified_intent,
                client_request=client_request,
            )
            tasks.append(skill.handle_client_request_message(intent_request.model_dump_json()))

        await asyncio.gather(*tasks)

        # Verify cache size is bounded
        assert len(skill.intent_requests) <= CACHE_SIZE_LIMIT
        assert len(skill.processed_messages) == message_count
...
class TestPerformanceUnderLoad:
    ...
    async def test_high_throughput_message_processing(self):
        ...
        for i in range(message_count):
            client_request = intent.ClientRequest(
                id=uuid.uuid4(), text=f"load test message {i}", room="load_test_room", output_topic="test/load_output"
            )
            classified_intent = intent.ClassifiedIntent(
                intent_type=intent.IntentType.DEVICE_ON,
                confidence=0.95,
                entities={},
                raw_text=f"load test message {i}",
            )
            intent_request = intent.IntentRequest(
                classified_intent=classified_intent,
                client_request=client_request,
        ...
        assert len(skill.intent_requests) <= skill.intent_requests.max_size
